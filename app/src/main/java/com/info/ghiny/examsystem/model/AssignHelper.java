...
public class AssignHelper {
    ...
    private TakeAttdActivity assignAct;
    ...
    private final DialogInterface.OnClickListener updateListener =
            new DialogInterface.OnClickListener(){
                ...
                public void onClick(DialogInterface dialog, int which) {
                    updateNewCandidate();
                    assignAct.setTableView("");
                    assignAct.setCandidateView("", "", "");
                    dialog.cancel();
                }
            };
    ...
    public void setAssignAct(TakeAttdActivity assignAct) {
        this.assignAct = assignAct;
    }
    protected TakeAttdActivity getAssignAct(){
        return assignAct;
    }
    ...
    public void tryAssignScanValue(String scanStr) throws ProcessException{
        if(scanStr.length() < 4 && scanStr.length() > 0){
            checkTable(scanStr);
            assignAct.setTableView(tempTable.toString());
        } else if(scanStr.length() == 10){
            checkCandidate(scanStr);
            assignAct.setCandidateView(tempCdd.getExamIndex(), tempCdd.getRegNum(), tempCdd.getPaper().toString());
        } else {
